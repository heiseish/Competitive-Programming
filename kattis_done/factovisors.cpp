...
vi primeFactors(ll N) {   // remember: vi is vector of integers, ll is long long
	...
	while ((N != 1) && (PF*PF <= N)) {   // stop at sqrt(N), but N can get smaller
		while (N%PF == 0) { N /= PF; factors.pb(PF); }      // remove this PF
...
bool check(int x, int thres) {
	int cur = 1;
	int ans = 0;
	while((double) 1.0 * cur <= (double) log(n)/log(x)) {
		ans += (int) floor(n/pow(x, cur++));
...
int main() {
	...
	while(scanf("%d %d", &n, &m) != EOF) {
		vi ans = primeFactors(m);
		bool can = true;
		while(!ans.empty()) {
			int next = ans.back();ans.pop_back();
			int ct = 1;
			while(ans.back() == next) {
				ans.pop_back();
				ct++;
