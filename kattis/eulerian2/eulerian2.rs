...
fn solve<R: BufRead, W: Write>(mut input: FastInput<R>, mut w: W) -> Option<()> {
    ...
    let mut graph = vec![HashMap::new(); n as usize + 5usize];
    for _ in 1..=m {
        let a: usize = input.token();
        let b: usize = input.token();
        *graph[a].entry(b).or_insert(0) += 1;
    }

    let sol: usize = input.token();
    ...
    for i in 1..=sol + 1 {
        ...
        if !graph[prev].contains_key(&current) {
            write!(w, "{}\n", i - 1);
            return Some(());
        }
        let e = graph[prev].get_mut(&current)?;
        *e -= 1;
        if *e == 0 {
