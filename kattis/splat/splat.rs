...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut w: W) -> Option<()> {
    ...
    for _ in 0..tc {
        ...
        let mut a = Vec::new();
        for _ in 0..n {
            let x: f32 = input.next();
            let y: f32 = input.next();
            let mut r: f32 = input.next();
            let c: String = input.next();
            r = (r / PI).sqrt();
            a.push(Color { x, y, r, c });
        }
        let q: u32 = input.next();
        for _ in 0..q {
            let x: f32 = input.next();
            let y: f32 = input.next();
            ...
            let mut ans = false;
            for c in a.iter().rev() {
                if ((c.x - x).powi(2) + (c.y - y).powi(2)).sqrt() <= c.r {
                    ans = true;
                    write!(w, "{}\n", c.c);
                    break;
                }
