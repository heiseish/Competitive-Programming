...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) -> Option<()> {
    ...
    let mut f = [[0u32; 505]; 505];
    let mut marked = [[false; 505]; 505];

    let mut islands = Vec::with_capacity(h * w);
    for i in 0..h {
        for j in 0..w {
            f[i][j] = input.next();
            islands.push((i, j, f[i][j]));
        }
    }

    let mut ans = 0;
    islands.sort_by(|a, b| b.2.cmp(&a.2));

    for il in &islands {
        if il.2 <= 1 || marked[il.0][il.1] {
            continue;
        }

        let mut q = VecDeque::with_capacity(h * w);
        q.push_back(il.to_owned());
        marked[il.0][il.1] = true;
        ...
        loop {
            ...
            for (delta_x, delta_y) in [(-1, 0), (1, 0), (0, 1), (0, -1)] {
                let x = front.0 as i32 + delta_x;
                let y = front.1 as i32 + delta_y;
