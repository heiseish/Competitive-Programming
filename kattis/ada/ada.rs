...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut w: W) -> Option<()> {
    ...
    let mut tiers = vec![v.to_owned()];
    loop {
        let nxt = v.windows(2).map(|w| w[1] - w[0]).collect::<Vec<i32>>();
        tiers.push(nxt.to_owned());
        if nxt.windows(2).all(|w| w[0] == w[1]) {
            break;
        }
        v = nxt;
    }
    for i in (0i32..=(tiers.len() as i32 - 2)).rev() {
        let prev = tiers[i as usize].last()?.to_owned();
        let higher = tiers[(i + 1) as usize].last()?.to_owned();
        tiers[i as usize].push(prev + higher);
    }
    write!(w, "{} {}\n", tiers.len() as i32 - 1, tiers[0].last()?);
    Some(())
}
