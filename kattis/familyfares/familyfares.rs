...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) -> Option<()> {
    ...
    let g: u64 = input.next();
    let mut ps = [0usize; 105];
    for i in 0..p {
        ps[i] = input.next::<usize>() - 1;
    }
    let mut d = [[u32::MAX as u64; 1003]; 1003];
    let mut adj = vec![vec![]; n];
    for i in 0..m {
        let a: usize = input.next();
        let b: usize = input.next();
        let c: u64 = input.next();
        adj[a - 1].push((b - 1, c));
        adj[b - 1].push((a - 1, c));
    }
    dijkstra(0, &mut d[0], &adj);
    for i in 0..p {
        dijkstra(ps[i], &mut d[ps[i]], &adj)
    }
    for i in 0..n {
        for j in 0..n {
...
fn dijkstra(start: usize, dist: &mut [u64], adj_list: &Vec<Vec<(usize, u64)>>) {
    if dist[start] == 0 {
        return;
    }
