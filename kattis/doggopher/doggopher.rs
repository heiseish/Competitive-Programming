...
fn solve<R: BufRead, W: Write>(mut input: FastInput<R>, mut w: W) -> Option<()> {
    let gx: f32 = input.token();
    let gy: f32 = input.token();

    let dx: f32 = input.token();
    let dy: f32 = input.token();
    let mut s = String::new();
    while input.read_line(&mut s) {
        let mut v = s.split(' ');
        let hx = v.next()?.parse::<f32>().ok()?;
        let hy = v.next()?.parse::<f32>().ok()?;
        if ((hy - gy).abs().powi(2) + (hx - gx).abs().powi(2)).sqrt() * 2.
            <= ((hy - dy).abs().powi(2) + (hx - dx).abs().powi(2)).sqrt()
        {
            write!(
                w,
    ...
    write!(w, "The gopher cannot escape.\n");
    Some(())
}
