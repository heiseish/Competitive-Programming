...
fn solve<R: BufRead, W: Write>(mut input: FastInput<R>, mut w: W) -> Option<()> {
    ...
    while input.read_line(&mut s) {
        ...
        let mut is_surjective = true;

        let mut domain: HashSet<String> = s.split(' ').skip(1).map(|x| x.to_owned()).collect();
        s.clear();
        input.read_line(&mut s);
        let mut codomain: HashSet<String> = s.split(' ').skip(1).map(|x| x.to_owned()).collect();
        s.clear();

        let tc: u16 = input.token();
        for _ in 1..=tc {
            while !input.read_line(&mut s) {}
            let sp: Vec<&str> = s.split(' ').collect();
            if !domain.contains(sp[0] as &str) {
                is_fn = false;
            }

            domain.remove(sp[0] as &str);
            let tmp = sp.last()?;
            if !codomain.contains(tmp as &str) {
                is_injective = false;
            }
            codomain.remove(tmp as &str);
            s.clear();
        }

        is_surjective &= codomain.len() == 0;

        if !is_fn {
            write!(w, "not a function\n");
        } else if !is_injective && !is_surjective {
            write!(w, "neither injective nor surjective\n");
        } else if is_injective && is_surjective {
            write!(w, "bijective\n");
