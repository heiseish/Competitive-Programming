...
//! Count the number of ways to assign m numbers, such that their sum is n
fn count(n: usize, m: usize) -> u64 {
    if m == 0 {
        return 0;
    }
    let mut dp = [[0; 31]; 31];
...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) -> Option<()> {
    ...
    loop {
        ...
        // for equal sum, find those that are lexicographically smaller
        let mut acc = 0;
        for i in 0..n {
            for j in 0..a[i] {
                ans += count((s - acc - j) as usize, n - i - 1);
            }
            acc += a[i];
        }
        ans += 1; // same set up as current
        write!(output, "{}\n", ans);
    }
    None
}
