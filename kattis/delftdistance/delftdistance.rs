...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) -> Option<()> {
    // read in int/float
    let mut dp = [[0u16; 1405]; 1405];
    let mut cc = [[0u16; 705]; 705];
    ...
    for i in 1..=(h * 2) {
        for j in 1..=(w * 2) {
            ...
            if i % 2 == 0 && j % 2 == 0 {
                ...
                dp[i][j] = max(dp[i][j - 1], dp[i - 1][j - 1] + cc[i / 2 - 1][j / 2]);
            } else {
                dp[i][j] = max(dp[i - 1][j], dp[i - 1][j - 1] + cc[i / 2][j / 2 - 1]);
            }
        }
    }
    const r: f32 = 10.0 * PI / 4.0;
    write!(
        output,
        "{:.10}\n",
        ((w + h) * 10) as f32 + dp[h * 2][w * 2] as f32 * (r - 10.0)
    );
    None
}
