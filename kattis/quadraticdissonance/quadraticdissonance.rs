...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) -> Option<()> {
    // read in int/float
    let a: f32 = input.next();
    let b: f32 = input.next();
    let c: f32 = input.next();
    let d: f32 = input.next();

    let comp = |x: f32| -> f32 { (x * x + a * x + b).max(x * x + c * x + d) };
    let mut choices = vec![];
    let first = -a / 2.0;
    let second = -c / 2.0;
    choices.push((first, comp(first)));
    choices.push((second, comp(second)));
    if a != c {
        let third = (d - b) / (a - c);
        choices.push((third, comp(third)));
    }

    choices.sort_by(|a, b| a.1.partial_cmp(&b.1).unwrap());

    write!(output, "{:.10} {:.10}", choices[0].0, choices[0].1);
    None
}
