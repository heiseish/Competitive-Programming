...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    // read in int/float
    let mut g = vec![vec![0; 1_003]; 1_003];
    'outer: for _ in 0..(input.next::<usize>()) {
        ...
        // dp[1].sort();
        // dp[1].reverse();
        fn get(l: usize, idx: usize, i: usize, dp: &[Vec<i64>], g: &[Vec<i64>]) -> i64 {
            if l + 1 < i {
                dp[l][idx] + g[l + 2][i]
            } else {
                dp[l][idx]
        ...
        for i in 1..=n {
            ...
            while !h.is_empty() {
                ...
                let (val, row_idx, idx) = h.pop().unwrap();

                dp[i].push(val);

                if row_idx <= n && idx + 1 < dp[row_idx].len() {
                    h.push((get(row_idx, idx + 1, i, &dp, &g), row_idx, idx + 1))
                }
            }
        }
        dbg!(&dp[0]);
