...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    // read in int/float
    let mut p = vec![0; 100_005];
    let mut ans = vec![0; 100_005];
    'outer: for _ in 0..(input.next::<usize>()) {
        let n: usize = input.next();
        p[0..n].iter_mut().for_each(|x| *x = input.next());
        for i in 0..(n / 2) {
            ans[2 * i] = i + 1;
        }
        for i in (n / 2)..n {
        ...
        let mut cnt = 0;
        for i in 1..(n - 1) {
            if (p[i] + ans[i]) > (p[i - 1] + ans[i - 1])
                && (p[i] + ans[i]) > (p[i + 1] + ans[i + 1])
            {
                cnt += 1;
            }
        }
