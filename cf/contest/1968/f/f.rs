...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    ...
    'outer: for _ in 0..(input.next::<usize>()) {
        ...
        for i in 1..=n {
            p[i] = a[i] ^ p[i - 1];
            hm.entry(p[i]).or_default().push(i);
        }
        for _ in 0..q {
            let mut l: usize = input.next();
            let mut r: usize = input.next();
            if p[r] == p[l - 1] {
                write!(output, "YES\n");
            } else {
                let tpos = &hm[&p[l - 1]];
                ...
                let tpos_res = tpos.partition_point(|&x| x < r);
                let spos_res = spos.partition_point(|&x| x < l);
                if tpos_res == 0 || spos_res == spos.len() {
                    write!(output, "NO\n");
                } else if tpos[tpos_res - 1] > spos[spos_res] {
                    write!(output, "YES\n");
                } else {
                    write!(output, "NO\n");
                }
