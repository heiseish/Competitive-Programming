...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    // read in int/float
    let mut a = [0i64; 200_007];
    let mut p = [0usize; 200_007];
    'outer: for _ in 0..(input.next::<usize>()) {
        let n: usize = input.next();
        let k: i32 = input.next();
        let pb: usize = input.next();
        let ps: usize = input.next();
        p[1..=n].iter_mut().for_each(|x| *x = input.next());
        ...
        let mut x = pb;
        let nn = k.min(n as i32) as usize;
        for i in 0..nn {
            bb = bb.max(cb + a[x] * (k - i as i32) as i64);
            cb += a[x];
            x = p[x];
        }
        ...
        let mut x = ps;
        for i in 0..nn {
            bs = bs.max(cs + a[x] * (k - i as i32) as i64);
            cs += a[x];
            x = p[x];
        }
        if bb == bs {
