...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) -> Option<()> {
    ...
    let mut a = [0i32; 200_007];
    let mut b = [0i32; 200_007];
    ...
    for _ in 0..t {
        ...
        let mut ssb: HashMap<i32, i32> = HashMap::new();
        for i in 0..m {
            b[i] = input.next();
            *ssb.entry(b[i]).or_default() += 1;
        }

        let mut ssa: HashMap<i32, i32> = HashMap::new();
        let mut num_sim = 0;
        let mut ans = 0;
        for i in 0..m {
            if ssb.contains_key(&a[i]) {
                *ssa.entry(a[i]).or_default() += 1;
            } else {
                continue;
            }
        ...
        for to_pop in 0..(n - m) {
            if ssa.contains_key(&a[to_pop]) {
                if ssa[&a[to_pop]] <= ssb[&a[to_pop]] {
                    num_sim -= 1;
                }
                ssa.entry(a[to_pop]).and_modify(|x| *x -= 1);
            }

            let to_insert = to_pop + m;
            if ssb.contains_key(&a[to_insert]) {
                *ssa.entry(a[to_insert]).or_default() += 1;

                if ssa[&a[to_insert]] <= ssb[&a[to_insert]] {
                    num_sim += 1;
                }
            }
            if num_sim >= k {
