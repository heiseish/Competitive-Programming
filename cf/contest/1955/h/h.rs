...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    ...
    'outer: for _ in 0..t {
        ...
        let mut cover = vec![vec![0i32; R]; k];
        for tower in 0..k {
            let mut x: usize = input.next();
            let mut y: usize = input.next();
            let mut p: i32 = input.next();
            x -= 1;
            y -= 1;
            for r in 0..R {
                cover[tower][r] = 0;
            ...
            for r in 0..R {
                for i in 0..n {
                    for j in 0..m {
                        if board[i][j] == '#' && sq(x, i) + sq(y, j) <= sq(r, 0usize) {
                            cover[tower][r] += p;
                        }
                    }
                }
        ...
        for tower in 0..k {
            ...
            for mask in 0..MASK {
                for r in 0..R {
                    ...
                    let nxt_mask = mask | (1 << r);
                    newdp[nxt_mask] = newdp[nxt_mask].max(dp[mask] + cover[tower][r]);
                }
            }
            dp = newdp;
        }
        let mut maxh = 0;
        for mask in 0..MASK {
            let mut base = dp[mask];
            for r in 0..R {
                if mask & (1 << r) > 0 {
                    base -= 3i32.pow(r as u32);
                }
            }
            maxh = maxh.max(base);
        }
