...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) -> Option<()> {
    ...
    for tc in 0..t {
        ...
        let mut ss: HashMap<usize, i32> = HashMap::default();

        for i in 0..n.pow(2) {
            a[i] = input.next();
            ss.entry(a[i]).and_modify(|x| *x += 1).or_insert(1);
        }
        let mut b = &mut a[0..n.pow(2)];
        b.sort();

        let a11 = b[0];
        ss.entry(a11).and_modify(|x| *x -= 1);
        ...
        'outer: for i in 0..n {
            for j in 0..n {
                ...
                let rem = ss.entry(a11 + val).or_default();
                *rem -= 1;
