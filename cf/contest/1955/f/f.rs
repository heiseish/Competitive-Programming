...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) -> Option<()> {
    ...
    for _ in 0..t {
        let ones: u32 = input.next();
        let twos: u32 = input.next();
        let threes: u32 = input.next();
        let fours: u32 = input.next();

        let mut initial_lost = fours % 2;
        if (ones + threes) % 2 == 1 || (twos + threes) % 2 == 1 {
            initial_lost += 1;
        }

        let rem = ones + twos + threes + fours - initial_lost;
        let ans = rem / 2;
