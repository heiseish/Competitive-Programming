...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    ...
    let mut ss = [0u32; 5007];
    let mut a = [[0i32; 105]; 105];
    let mut dp = [[0i32; 105]; 105];
    'outer: for _ in 0..t {
        ...
        let mm = gcd(a[0][0], a[n - 1][m - 1]);
        let mut divs = vec![1, mm];
        {
            let mut div = 2;
            while div <= mm / div {
                if mm % div == 0 {
        ...
        for div in divs.iter().rev() {
            ...
            for i in 0..n {
                for j in 0..m {
                    ...
                    let can_div = (a[i][j] % div == 0) as i32;
                    if i > 0 && j > 0 {
                        dp[i][j] = (dp[i - 1][j] & can_div).max(dp[i][j - 1] & can_div)
                    } else if i > 0 {
                        dp[i][j] = (dp[i - 1][j] & can_div);
                    } else {
                        dp[i][j] = (dp[i][j - 1] & can_div);
                    }
                }
            }
            if dp[n - 1][m - 1] == 1 {
