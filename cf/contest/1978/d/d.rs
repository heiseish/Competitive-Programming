...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    'outer: for _ in 0..(input.next::<usize>()) {
        ...
        let mut lh: BinaryHeap<i64> = BinaryHeap::new();
        for i in 1..=n {
            rh.push((a[i], i));
        }
        ...
        for i in 1..=n {
            // prune rh
            while !rh.is_empty() && rh.peek().unwrap().1 <= i {
                rh.pop();
            }
            ...
            // dbg!(i, ans, num, &rh);
            if !rh.is_empty() && num < rh.peek().unwrap().0 {
                ans += (i - 1);
                num = ap[i] + c;

                // dbg!(i, ans, num, rh.peek().unwrap().0);
                if !rh.is_empty() && num < rh.peek().unwrap().0 {
                    ans += 1;
                }
                // dbg!(i, ans, num, rh.peek().unwrap().0);
            } else if !lh.is_empty() && num <= *lh.peek().unwrap() {
                ans += (i - 1);
            }
