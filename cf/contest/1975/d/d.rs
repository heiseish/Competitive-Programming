...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    ...
    let mut c = vec![0; 200_005];
    let mut vis = vec![false; 200_005];
    let mut dist = vec![0; 200_005];
    let mut visc = vec![vec![false; 200_005]; 2];
    'outer: for _ in 0..(input.next::<usize>()) {
        ...
        if a == b {
            ...
            let mut p = vec![vec![0; n + 2]; 2];

            while !q.is_empty() {
                let (idx, cost, col) = q.pop_front().unwrap();
                visc[col][idx] = true;
                if visc[col ^ 1][idx] && s == 0 {
                    if col == 1 {
                        ans = cost;
                        s = p[1][idx];
                    } else {
                ...
                for &nb in &g[idx] {
                    ...
                    p[col][nb] = idx;
                    q.push_back((nb, cost + 1, col));
                }
            }
        }

        fn dfs(u: usize, g: &[Vec<usize>], vis: &mut [bool], dist: &mut [usize], dt: usize) {
            vis[u] = true;
            dist[u] = dt;
            ...
            for &v in &g[u] {
                ...
                dfs(v, g, vis, dist, dt + 1);
            }
        }
        dfs(s, &g, &mut vis, &mut dist, 0);
        write!(
            output,
            "{}\n",
            ans + (n - 1) * 2 - dist[1..=n].into_iter().max().unwrap()
        );
    }
}
