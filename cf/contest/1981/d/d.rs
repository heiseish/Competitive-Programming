...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    // read in int/float
    let primes = sieve::<1_000_006>();
    let n = primes.len();
    let M = 1_000_000;

    let mut ans = vec![1, 1, 2, 2, 3, 3, 1];
    let mut cur = 2;
    while ans.len() < M {
        let mut nxt = cur + 1;
        let mut nxt2 = cur + 2;
        ...
        ans.push(primes[nxt]);
        ans.push(primes[1]);
        ans.push(primes[nxt2]);
        ans.push(primes[nxt2]);
        let mut flag = 0;
        let mut pi = 2;
        let options = vec![primes[nxt], primes[nxt2]];
        for i in pi..=cur {
            ans.push(primes[i]);
            ans.push(options[flag]);
            flag ^= 1;
        }
        ans.push(options[flag]);
        ans.push(primes[0]);
    ...
    let mut ans2 = vec![1, 1, 2, 2];
    let mut cur = 1;
    while ans2.len() < M {
        let mut nxt = cur + 1;
        let mut nxt2 = cur + 2;

        ans2.push(primes[nxt]);
        ans2.push(primes[nxt]);
        ans2.push(primes[1]);
        ans2.push(primes[nxt2]);
        ans2.push(primes[nxt2]);
        let mut flag = 0;
        let mut pi = 2;
        let options = vec![primes[nxt], primes[nxt2]];
    ...
    let mut track1 = vec![0; M + 1];
    let mut track2 = vec![0; M + 1];
    let mut track1m = HashSet::new();
    let mut track2m = HashSet::new();
    for i in 0..M {
        track1m.insert(ans[i]);
        track2m.insert(ans2[i]);
        track1[i] = track1m.len();
        track2[i] = track2m.len();
    }

    'outer: for _ in 0..(input.next::<usize>()) {
        let n: usize = input.next();

        // if track1[n] < track2[n] {
        //     for i in 0..n {
        //         write!(output, "{} ", ans[i]);
        //     }
        // } else {
