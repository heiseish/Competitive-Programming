...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    // read in int/float
    fn ask<R: BufRead, W: Write>(d: i32, input: &mut InputReader<R>, output: &mut W) -> (i32, i32) {
        write!(output, "? {}\n", d);
        output.flush();
        let v: i32 = input.next();
        let u: i32 = input.next();
        (v, u)
    }
    fn get<R: BufRead, W: Write>(
        ...
        if n == 1 {
            let v = ask(0, input, output).0;
            return (v, v);
        }
        if n == 2 {
            let u = ask(0, input, output).0;
            let v = ask(0, input, output).0;
            nxt[u as usize] = v;
            return (u, v);
        }
        let t = ask((n - 2) as i32, input, output);
        let v = t.0;
        let ban = t.1;
        if ban != 0 {
            let res = get(n - 1, nxt, input, output);
            if ban != res.0 {
                nxt[v as usize] = res.0;
                return (v, res.1);
            } else {
                nxt[res.1 as usize] = v;
                return (res.0, v);
            }
        }
        ...
        let u = ask(0, input, output).0;
        nxt[u as usize] = v;
        let res = get(n - 2, nxt, input, output);
        nxt[v as usize] = res.0;
        (u, res.1)
    }
    'outer: for _ in 0..(input.next::<usize>()) {
        ...
        let ans = get(n, &mut nxt, &mut input, &mut output);
