...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    ...
    let MAXC = 100_001;
    let MAXD = 400_007;
    let mut diag = vec![BTreeSet::<(i64, usize)>::new(); MAXD];
    'outer: for _ in 0..(input.next::<usize>()) {
        ...
        let ops = vec![
            |a: i64, b: i64| -> usize { (a + b) as usize },
            |a: i64, b: i64| -> usize { (a - b) as usize },
        ];
        let mut op_flag = 0;

        let mut insert = |op_flag: usize, diag: &mut [BTreeSet<(i64, usize)>], y: &[i64]| {
            for i in 0..n {
                diag[ops[op_flag](x[i], y[i])].insert((x[i], i));
            }
        };
        let mut prune = |op_flag: usize, diag: &mut [BTreeSet<(i64, usize)>], y: &[i64]| {
            for i in 0..n {
                diag[ops[op_flag](x[i], y[i])].clear();
            }
        };
        for choice in 0..2 {
            insert(op_flag, &mut diag, &y);
            for i in 0..n {
                let mut it1 = diag[ops[op_flag](x[i], y[i])]
                    .range((Included((x[i] + d as i64 / 2, 0)), Unbounded));
                let nxt = it1.next();
                if nxt.is_none() || nxt.unwrap().0 != x[i] + d as i64 / 2 {
                    continue;
                }
                let nxt = nxt.unwrap().clone();
                if ops[op_flag](x[i], y[i]) + d < MAXD {
                    let mut it2 = diag[ops[op_flag](x[i], y[i]) + d]
                        .range((Included((x[i] + d as i64 / 2, 0)), Unbounded));
                    let nxt2 = it2.next();
                    if nxt2.is_some() && nxt2.unwrap().0 <= nxt.0 + d as i64 / 2 {
                        let nxt2 = nxt2.unwrap().clone();
                        write!(output, "{} {} {}\n", i + 1, nxt.1 + 1, nxt2.1 + 1);
                        prune(op_flag, &mut diag, &y);
                        continue 'outer;
                    }
                }
                if ops[op_flag](x[i], y[i]) as i64 - d as i64 >= 0 {
                    let mut it2 = diag[ops[op_flag](x[i], y[i]) - d]
                        .range((Included((x[i] - d as i64 / 2, 0)), Unbounded));
                    let nxt2 = it2.next();
                    if nxt2.is_some() && nxt2.unwrap().0 <= nxt.0 - d as i64 / 2 {
