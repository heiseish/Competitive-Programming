...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    ...
    for i in 1..=n {
        for j in ((a[i] as usize)..B).rev() {
            f[j] = add(f[j], f[j - a[i] as usize]);
        }
    }

    let mut ans = 0;
    for i in 0..B {
        ans = add(ans, mult(f[i], ((i + 1) / 2) as i64));
    }
    for i in 1..=n {
        for j in 0..a[i] {
            ans = add(ans, mult(f[j as usize], a[i] - (a[i] + j as i64 + 1) / 2));
        }
    }
    write!(output, "{}\n", ans);
}
