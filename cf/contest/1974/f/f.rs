...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    // read in int/float
    let mut x = vec![0; 200_005];
    let mut y = vec![0; 200_005];
    ...
    'outer: for _ in 0..(input.next::<usize>()) {
        ...
        ycoor.sort();
        let mut uidx = 0i32;
        let mut didx = n as i32 - 1;
        let mut lidx = 0i32;
        let mut ridx = n as i32 - 1;
        let mut is_al = true;
        ...
        for _ in 0..m {
            ...
            match dir {
                'U' => {
                    curubound += k;
                    while uidx < n as i32 && xcoor[uidx as usize].0 < curubound {
                        *point += can[xcoor[uidx as usize].1] as usize;
                        can[xcoor[uidx as usize].1] = false;
                        uidx += 1;
                    }
                }
                'D' => {
                    curdbound -= k;
                    while didx >= 0 && xcoor[didx as usize].0 > curdbound {
                        *point += can[xcoor[didx as usize].1] as usize;
                        can[xcoor[didx as usize].1] = false;
                        didx -= 1;
                    }
                }
                'L' => {
                    curlbound += k;
                    while lidx < n as i32 && ycoor[lidx as usize].0 < curlbound {
                        *point += can[ycoor[lidx as usize].1] as usize;
                        can[ycoor[lidx as usize].1] = false;
                        lidx += 1;
                    }
                }
                'R' => {
                    currbound -= k;
                    while ridx >= 0 && ycoor[ridx as usize].0 > currbound {
                        *point += can[ycoor[ridx as usize].1] as usize;
                        can[ycoor[ridx as usize].1] = false;
                        ridx -= 1;
                    }
                }
                _ => panic!(),
