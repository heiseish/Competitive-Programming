...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    ...
    'outer: for _ in 0..(input.next::<usize>()) {
        ...
        let th = h[1..=m].iter().sum::<usize>();
        let INF = i64::MAX / 2;

        let mut dp = vec![vec![-INF; th + 2]; m + 2];
        dp[0][0] = -x;
        for i in 1..=m {
            // do not do anything and accept salary
            for hh in 0..=th {
                dp[i][hh] = dp[i][hh].max(dp[i - 1][hh] + x);
            }
            // accept cost
            for hh in (h[i] as usize)..=th {
                if dp[i - 1][hh - h[i]] + x >= c[i] {
                    dp[i][hh] = dp[i][hh].max(dp[i - 1][hh - h[i]] + x - c[i]);
                }
            }
        }
        let mut ans = 0;
