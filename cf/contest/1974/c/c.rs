...
impl Track {
    fn get(&mut self, v: i64) -> i64 {
        self.cnt - *self.m.entry(v).or_default()
    }

    fn add(&mut self, v: i64) {
        self.cnt += 1;
        *self.m.entry(v).or_default() += 1;
    }
}

fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    // read in int/float
    let mut a = vec![0; 200_005];
    'outer: for _ in 0..(input.next::<usize>()) {
        let n: usize = input.next();
        a[1..=n].iter_mut().for_each(|x| *x = input.next());

        let mut cnt1 = HashMap::<(i64, i64), Track>::new();
        let mut cnt2 = HashMap::<(i64, i64), Track>::new();
        let mut cnt3 = HashMap::<(i64, i64), Track>::new();

        let mut ans = 0i64;

        for i in 1..=(n - 2) {
            let ccnt1 = cnt1.entry((a[i], a[i + 1])).or_default();
            let ccnt2 = cnt2.entry((a[i + 1], a[i + 2])).or_default();
            let ccnt3 = cnt3.entry((a[i], a[i + 2])).or_default();
