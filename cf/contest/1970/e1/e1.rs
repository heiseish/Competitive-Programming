...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    ...
    let mut f = vec![vec![0i64; m + 2]; n + 2];
    f[0][1] = 1;
    for i in 1..=n {
        let mut s1 = 0;
        let mut s2 = 0;
        for k in 1..=m {
            s1 = add(s1, mult(f[i - 1][k], s[k]));
            s2 = add(s2, mult(f[i - 1][k], l[k]));
        }
        for j in 1..=m {
            f[i][j] = add(f[i][j], mult(s1, l[j]));
            f[i][j] = add(f[i][j], mult(s2, s[j]));
            f[i][j] = add(f[i][j], mult(s1, s[j]));
        }
    }
