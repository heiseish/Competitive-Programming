...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    ...
    let qq = String::from(".Q");
    let bl = String::from(".B");
    let gs = String::from(".S");

    let mut rg = vec![vec![false; m + 2]; n + 2];
    let mut bg = vec![vec![false; m + 2]; n + 2];
    ...
    'outer: for tt in 0..t {
        ...
        let a: char = input.next();

        let pos = hh.get(&s).expect(&format!("cannot find pos for {}", s));

        match &a {
            'U' => {
                hh.insert(s, (pos.0 - 1, pos.1));
            }
            'D' => {
                hh.insert(s, (pos.0 + 1, pos.1));
            }
            'L' => {
                hh.insert(s, (pos.0, pos.1 - 1));
            }
            'R' => {
                hh.insert(s, (pos.0, pos.1 + 1));
            }
            'T' => {
                if rg[pos.0][pos.1] {
                    write!(output, "{} BLUE GOAL\n", tt);
                    bs += 1;
                } else if bg[pos.0][pos.1] {
                    write!(output, "{} RED GOAL\n", tt);
                    rs += 1;
                } else {
                }
            }
            'C' => {
                ...
                if b == gs {
                    let rgs = s.contains('R');

                    if rgs {
                        write!(output, "{} RED CATCH GOLDEN SNITCH\n", tt);
                        rs += 10;
                    } else {
                        write!(output, "{} BLUE CATCH GOLDEN SNITCH\n", tt);
                        bs += 10;
                    }
        ...
        elim.sort();
        for el in elim {
            write!(output, "{} {} ELIMINATED\n", tt, el);
            hh.remove(&el);
        }
    }
    write!(output, "FINAL SCORE: {} {}\n", rs, bs);
}
