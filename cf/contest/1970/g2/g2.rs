...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    ...
    'outer: for _ in 0..(input.next::<usize>()) {
        ...
        let mut f1 = vec![false; n + 7];
        let mut f2 = vec![false; n + 7];
        f1[0] = true;
        f2[0] = true;

        // loop through all the groups
        for i in 1..grp {
            for k in (grps_size[i]..=n).rev() {
                f2[k] |= f2[k - grps_size[i]];
            }
            for &v in &ble[i] {
                let half = szs[v];
                let other_half = grps_size[i] - szs[v];

                for k in (half..=n).rev() {
                    f2[k] |= f1[k - half];
                }
                for k in (other_half..=n).rev() {
                    f2[k] |= f1[k - other_half];
                }
            }
            for k in (grps_size[i]..=n).rev() {
                f1[k] |= f1[k - grps_size[i]];
            }
        }
        let mut mid = n / 2;
        while !f2[mid] {
