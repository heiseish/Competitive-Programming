...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    ...
    let mut s = vec![0; m];
    let mut l = vec![0; m];
    s[0..m].iter_mut().for_each(|x| *x = input.next());
    l[0..m].iter_mut().for_each(|x| *x = input.next());
    const M: usize = 100_007;
    let mut B = Matrix::<M, 2>::new();
    let mut C = Matrix::<2, M>::new();
    ...
    let mut base = Matrix::<M, 1>::new();
    base.data[0][0] = 1;

    let mut base = C * base;
    let mut cb = (C * B).pow(n - 1);
    let mut base = cb * base;
    let mut base = B * base;
    let mut ans = 0;

    for i in 0..m {
        ans = add(ans, base.data[i][0]);
    }
    write!(output, "{}\n", ans);
}
