...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    ...
    'outer: for _ in 0..(input.next::<usize>()) {
        ...
        let mut hh: HashMap<usize, Vec<usize>> = HashMap::new();
        let mut hidex: HashMap<usize, Vec<usize>> = HashMap::new();

        a[..n].iter_mut().for_each(|x| *x = input.next());
        for i in 0..n {
            (*hh.entry(a[i] >> 2).or_default()).push(a[i]);
            (*hidex.entry(a[i] >> 2).or_default()).push(i);
        }
        for (k, v) in hh.iter_mut() {
            v.sort();
            let hi = &hidex[k];
            for i in 0..v.len() {
                ans[hi[i]] = v[i];
