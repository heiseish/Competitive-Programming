...
fn oper(l: usize, r: usize, n: usize, a: &mut [i32], moves: &mut Vec<(usize, usize)>) {
    let mut cnt = [0; 20];
    for i in l..=r {
        if a[i] <= n as i32 {
            cnt[a[i] as usize] += 1;
...
fn build(l: usize, r: usize, n: usize, a: &mut [i32], moves: &mut Vec<(usize, usize)>) {
    if l == r {
        if a[l] > 0 {
            oper(l, r, n, a, moves);
        }
...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) -> Option<()> {
    ...
    a[0..n].iter_mut().for_each(|x| *x = input.next());
    ...
    for s in 0..(1 << n) {
        ...
        while l < n {
            if s & (1 << l) > 0 {
                let mut r = l;
                while r + 1 < n && (s & (1 << (r + 1)) > 0) {
                    r += 1;
                }
                tmp += (r - l + 1) as i32 * (r - l + 1) as i32;
                l = r;
            } else {
                tmp += a[l];
            }
    ...
    while l < n {
        if cur & (1 << l) > 0 {
            let r = l;
            let mut r = l;
            while r + 1 < n && (cur & (1 << (r + 1)) > 0) {
                r += 1;
            }
            build(l, r, n, &mut a[..], &mut moves);
            oper(l, r, n, &mut a[..], &mut moves);
            l = r;
        }
