    ...
    'outer: for _ in 0..(input.next::<usize>()) {
        ...
        if n == 2 {
            ...
            for i in 1..=n {
                if a[i] == 0 && a[i % n + 1] > 0 {
                    b[i % n + 1] = true;
                    b[(i + 2) % n + 1] = (a[(i + 2) % n + 1] as i64
                        > attack(a[(i + 1) % n + 1] as i64, a[i % n + 1] as i64));
                }
            }
        }
