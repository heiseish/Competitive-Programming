...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    ...
    'outer: for _ in 0..(input.next::<usize>()) {
        ...
        s3.reverse();
        s3.append(&mut s2);
        let mut rz = vec![0; 2 * n];
        buildz(n * 2, &s3, &mut rz);
        ...
        s2.append(&mut s3);
        s2.reverse();
        let mut lz = vec![0; 2 * n];
        buildz(n * 2, &s2, &mut lz);

        for i in 1..(n - 1) {
            if lz[2 * n - 1 - i] < i + 1 && rz[i + 1 + n] < n - i - 1 {
                write!(
                    output,
                    "YES\n2\n{} {}\n",
                    s[0..=i].iter().collect::<String>(),
                    s[(i + 1)..].iter().collect::<String>()
                );
                continue 'outer;
            }
        }
