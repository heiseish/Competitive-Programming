...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    ...
    'outer: for _ in 0..(input.next::<usize>()) {
        ...
        b.iter_mut().for_each(|x| *x = input.next());

        let m: usize = input.next();
        let mut d = vec![0; m];
        d.iter_mut().for_each(|x| *x = input.next());
        let mut h = HashMap::<i64, i64>::new();
        for i in 0..m {
            *h.entry(d[i]).or_default() += 1;
        }
        let mut can = false;

        for i in 0..n {
            if a[i] != b[i] {
                if !h.contains_key(&b[i]) || h[&b[i]] == 0 {
                    write!(output, "NO\n");
                    continue 'outer;
                }
                *h.get_mut(&b[i]).unwrap() -= 1;
            }
            can |= (b[i] == d[m - 1]);
        }
        for (k, v) in h {
