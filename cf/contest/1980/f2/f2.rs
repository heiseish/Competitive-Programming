...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    ...
    'outer: for _ in 0..(input.next::<usize>()) {
        ...
        let mut last_r_c = (0, 0, 0);
        let mut b = vec![0i64; k];
        let mut acum = 0;
        for i in 0..k {
            let hk = get_smallest_value(&map, 1).unwrap();
            let nxt = get_smallest_value(&map, 2);
            // dbg!(hk, nxt);
            ans += (hk - 1) * (fou[i].0 - last_r_c.0);
            acum += match nxt {
                Some(nxt) => (nxt - hk) * (fou[i].0 - last_r_c.0),
                None => (m - hk + 1) * (fou[i].0 - last_r_c.0),
            };
            // dbg!(acum);
            *map.get_mut(&fou[i].1).unwrap() -= 1;
            if map[&fou[i].1] == 0 {
                map.remove(&fou[i].1);
                if hk == fou[i].1 {
                    // dbg!(acum);
