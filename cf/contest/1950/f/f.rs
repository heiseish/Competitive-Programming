...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    ...
    'outer: for tc in 0..(input.next::<usize>()) {
        let mut a: i32 = input.next();
        let mut b: i32 = input.next();
        let mut c: i32 = input.next();
        ...
        match (a, b, c) {
            ...
            _ => {
                let row_a = (a as f64).log(2.0) as i32;
                let mut ans = row_a - 1;
                let rem = a - 2i32.pow(row_a as u32) + 1;
                if rem > 0 {
                    ans += 1;
                }
                let single_branch_from_prev = 2i32.pow(row_a as u32) - rem;
                let single_branch_from_nxt = rem << 1;

                // dbg!(row_a, rem, single_branch_from_prev, single_branch_from_nxt);
                if c != single_branch_from_prev + single_branch_from_nxt {
                    write!(output, "-1\n");
                    continue 'outer;
                } else {
                    let fill1 = b.min(single_branch_from_prev);

                    b -= single_branch_from_prev;
                    ans += b / (single_branch_from_prev + single_branch_from_nxt);
                    b %= (single_branch_from_prev + single_branch_from_nxt);
                    ans += (b > 0) as i32;
                    ans += 1;
                }
