...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    // read in int/float
    let mut initials = vec![String::from("1"), String::from("0")];
    for i in 0..7 {
        let mut nxt = initials.clone();
        for mut v in initials {
            let mut one = v.clone();
            one.push('1');
            nxt.push(one);
            v.push('0');
    ...
    let mut initials = initials
        ...
        .filter(|&x| x <= 100_000 && x > 1)
        .collect::<BTreeSet<i32>>()
        .into_iter()
        .collect::<Vec<_>>();
    dbg!(&initials);
    'outer: for _ in 0..(input.next::<usize>()) {
        let mut n: i32 = input.next();
        while n != 1 {
            let mut can = false;
            for &v in initials.iter().rev() {
                if n % v == 0 {
                    n /= v;
                    can = true;
                    break;
