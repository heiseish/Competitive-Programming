...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    ...
    let mut ts = twosat::TwoSat::new(n << 1);
    for i in 0..n {
        X[i] = input.next();
        Y[i] = input.next();
        ts.add_disjunction(i, false, i + n, false);
    }
    for i in 0..n {
        for j in 0..n {
            if i == j {
            ...
            if (X[i] - Y[j]).abs() < d {
                ts.add_disjunction(i, true, j + n, true);
            }
            if (Y[i] - X[j]).abs() < d {
                ts.add_disjunction(i + n, true, j, true);
            }
            if (Y[i] - Y[j]).abs() < d {
                ts.add_disjunction(i + n, true, j + n, true);
            }
        }
    }
    let (can, assignment) = ts.solve();
    if !can {
        write!(output, "No\n");
        return;
    }
