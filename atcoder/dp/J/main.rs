...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    ...
    let mut dp = vec![vec![vec![-1.0; 303]; 303]; 303];
    fn recurse(i: usize, j: usize, k: usize, n: usize, dp: &mut [Vec<Vec<f64>>]) -> f64 {
        if dp[i][j][k] >= 0.0 {
            return dp[i][j][k];
        }
        if i == 0 && j == 0 && k == 0 {
        ...
        let mut ans = 0.0;
        if i > 0 {
            ans += recurse(i - 1, j, k, n, dp) * i as f64;
        }
        if j > 0 {
            ans += recurse(i + 1, j - 1, k, n, dp) * j as f64;
        }
        if k > 0 {
            ans += recurse(i, j + 1, k - 1, n, dp) * k as f64;
        }
        ans += n as f64;
    ...
    write!(output, "{:.20}\n", recurse(Xs[1], Xs[2], Xs[3], n, &mut dp));
}
