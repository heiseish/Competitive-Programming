...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    ...
    fn topo(u: usize, g: &[Vec<usize>], vis: &mut [bool], topos: &mut Vec<usize>) {
        vis[u] = true;
        for &v in &g[u] {
            if vis[v] {
                continue;
    ...
    for u in topos {
        for &prev in &rg[u] {
            dp[u] = dp[u].max(dp[prev] + 1);
        }
    }
    write!(output, "{}\n", dp[1..=n].iter().max().unwrap());
}
