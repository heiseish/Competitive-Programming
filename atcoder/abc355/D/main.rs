...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    ...
    let mm = compress(ak, Some(1));
    let l = l.into_iter().map(|x| mm[&x]).collect::<Vec<_>>();
    let r = r.into_iter().map(|x| mm[&x]).collect::<Vec<_>>();

    let sz = mm.len() + 3;
    let mut tree = fw::FenwichTree::new(sz);
    ab.sort();
    for (ll, rr) in ab {
        ans += tree.rsq(mm[&ll] as i64, (sz - 1) as i64);
        tree.update(mm[&rr] as usize, 1);
    }
    write!(output, "{}\n", ans);
}
