...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    ...
    let mut g = vec![0; n + 1];
    let mut ret = vec![-2; n + 1];
    for i in 1..=n {
        g[i] = input.next();
    }
    let mut hs = HashSet::new();
    fn dfs(u: usize, g: &[usize], cnt: &mut [usize], ret: &mut [i64], hs: &mut HashSet<usize>) {
        hs.insert(u);
        let nxt = g[u];
        ret[u] = -1;
        if ret[nxt] != -2 {
    ...
    for u in 1..=n {
        if ret[u] == -2 {
            cnt[u] = 0;
            dfs(u, &g, &mut cnt, &mut ret, &mut hs);
            hs.clear();
        }
    }
    // for i in 1..=n {
