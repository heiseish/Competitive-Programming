...
mod lazy_st {
    ...
    pub fn f_map_act(f: &Act, x: &Node) -> Node {
        Node {
            ...
            b: x.b + f.b * x.c,
            c: x.c,
            s: x.s + x.a * f.b + x.b * f.a + x.c * f.a * f.b,
        }
    }
        ...
        FCA: FnOnce(&A, &A) -> A + Copy,
    {
        ...
        pub fn get(&mut self, x: usize, y: usize) -> D {
                ...
                FCA: FnOnce(&A, &A) -> A + Copy,
            {
                ...
                st.push(v, l, r, fmap, fcompose);
                if l >= x && r <= y {
                    return st.t[v]; // read in int/float
                }
                let mid = (l + r) >> 1;
                fc(
...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    ...
    let mut a = vec![0i64; n + 6];
    let mut b = vec![0i64; n + 6];
    let mut c = vec![Node::default(); n + 6];
    a[1..=n].iter_mut().for_each(|x| *x = input.next());
    b[1..=n].iter_mut().for_each(|x| *x = input.next());
    for i in 1..=n {
        c[i] = Node {
            a: a[i].into(),
            b: b[i].into(),
            c: 1.into(),
            s: mint::from(a[i]) * mint::from(b[i]),
        };
    }
