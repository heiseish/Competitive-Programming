...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    ...
    aa.sort_by(|&x, &y| x.1.cmp(&y.1));
    ac.sort_by(|&x, &y| x.2.cmp(&y.2));

    let mut aai = n - 1;
    let mut aci = n - 1;
    loop {
        while ac[aci].0 != aa[aai].0 && aci > 0 {
            chosen[ac[aci].0] = false;
            aci -= 1;
        }
        // dbg!(aci, aai);
        if ac[aci].0 != aa[aai].0 {
            break;
        }
        ...
        aci -= 1;
        aai -= 1;
        while aci > 0 && !chosen[ac[aci].0] {
            aci -= 1;
        }
        while aai > 0 && !chosen[aa[aai].0] {
            aai -= 1;
        }
