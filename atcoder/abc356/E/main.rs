...
fn solve<R: BufRead, W: Write>(mut input: InputReader<R>, mut output: W) {
    ...
    let mut counter: BTreeMap<usize, i64> = BTreeMap::new();
    for i in 0..n {
        f[a[i]] += 1;
        *counter.entry(a[i]).or_default() += 1;
    }
    for i in 1..M {
        f[i] += f[i - 1];
    }
    ...
    for (k, v) in counter {
        ans += v * (v - 1) / 2;
        let mut cur = 1;
        while k as i64 * cur <= M as i64 {
            ans += v
                * cur
                * (f[(k * (cur + 1) as usize - 1).min(M - 1)] - f[(k * cur as usize - 1).max(k)]);
            cur += 1;
        }
    }
    write!(output, "{}\n", ans);
